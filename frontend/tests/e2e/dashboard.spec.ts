...
/**
 * Dashboard page (pages/DashboardPage.tsx): equity stat cards, the risk summary card, and
 * the positions-at-a-glance table, all sourced from GET /api/portfolio + GET
 * /api/portfolio/risk against the real backend (see playwright.config.ts's fixture-mode
 * webServer). Doesn't assert a specific position count/total (the shared e2e database may
 * already hold rows left by portfolio.spec.ts depending on run order) — only that every
 * section actually renders real data instead of staying stuck on a loading/error state.
 */
test('dashboard renders equity summary, risk summary, and positions at a glance', async ({
  page,
}) => {
  await page.goto('/')
