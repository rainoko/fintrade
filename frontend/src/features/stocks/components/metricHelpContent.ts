...
export const valueZoneHelp = {
  ...
  elderContext:
    'EMA(13)/EMA(26) no longer drive the Tide (Screen 1) directly -- that’s now the weekly Impulse System color (weekly EMA(13) direction + weekly MACD-Histogram direction, docs/Analyse.md §2). This pair still feeds the daily Impulse System gate (EMA(13) is one of its two daily inputs) and the Elder-Ray baseline elsewhere on this page (docs/Analyse.md §3-4) -- also plotted as the two solid trend lines on this same chart. Elder calls the zone between them a good swing-trade profit target ("the value zone on a weekly chart presents a good target," ch. 38/53) -- this app doesn’t yet compute an explicit target price from it (a separate, still-open task), so today this shading is informational, not a live target or stop input.',
  interpretValue(
    ema13: number | null | undefined,
    ema26: number | null | undefined,
  ): string {
