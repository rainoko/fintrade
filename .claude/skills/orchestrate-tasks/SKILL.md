...
Also run `python3 scripts/validate_tasks.py` (the `validate-task-board` skill) once before dispatching anything — a malformed task JSON or an `index.json` mirror drift can send `next-task`/`task-worker` down the wrong path for whichever task it touches. Unlike the auth check above, a non-zero result here doesn't block the whole run, and this preflight step never edits `docs/tasks/` itself to fix what it finds: `main`'s branch-protection ruleset rejects every direct push, including a bookkeeping-only one (per `CLAUDE.md`), so there is no such thing as a quick fix "on the current branch (`main`)" here — any real fix has to go through a commit, a push, and a PR, the same as `revalidate-done-tasks`' own "Handling a task that no longer holds" section does it (a dedicated branch, e.g. `chore/validate-task-board-<YYYY-MM-DD>`, the fix committed and pushed, then `gh pr create` opening a direct board-authoring PR a human reviews and merges themselves — not a run through the full task-worker/pr-reviewer/pr-decision/pr-merger pipeline, since there's no application code here for that pipeline to review). Given that cost, treat this step as report-only: note every violation found (which task id(s), what's wrong) and proceed with the loop regardless — a validator finding on one task's JSON shouldn't stall every other ready task. Only actually open a fix PR yourself (following the mechanics above) if a violation is severe enough to concretely block the task about to be dispatched next (e.g. an `index.json` mirror drift on that exact task); otherwise leave it for a later dedicated `validate-task-board` pass or a human to pick up.
