...
5. Work the checklist top to bottom. For each genuine judgment call not pinned down by the docs (per `CLAUDE.md`'s decision-memory rule), decide it yourself and append a `decisions` entry — decide and move on, don't ask. **Timestamping a `decisions` entry (or any other timestamp you write into a task JSON — `git.pushed_at` included)**: get the real current time by running `date -u +%Y-%m-%dT%H:%M:%SZ` in a Bash call immediately before you write it, and use that exact output verbatim. Never estimate, round to the minute, or otherwise guess an ISO 8601 timestamp — a placeholder value (e.g. two entries exactly one second apart) looks fabricated and undermines the timestamp's whole purpose as a record of when the judgment call was actually made, even though it isn't currently caught by `scripts/validate_tasks.py`. This is the same convention `pr-reviewer`/`pr-decision` already follow for `review.reviewed_at`/`pr_decision.confirmed_at`.
6. **If you hit something only the user can actually decide** (a missing external credential, a conflicting requirement, a product call the docs don't and can't cover) — do not stop and ask. Instead: commit whatever partial progress is safe to keep, append an entry to the task's `questions` array (`{"timestamp": "<ISO 8601 UTC>", "raised_by": "task-worker", "question": "...", "context": "..."}`) — capture that `timestamp` the same way as step 5's `decisions` entries: real `date -u` output, never estimated — set `state` to `"waiting_input"`, mirror `index.json`, and end your turn — then move on (or report back to the orchestrator so it moves on) to the next task. This is for things genuinely outside your authority, not for ordinary implementation choices — most ambiguity should become a `decisions` entry, not a `questions` entry.
