...
5a. **Run the persisted e2e suite for UI-facing work.** Whenever step 5 applies, also load and run the `e2e-test` skill (`frontend/tests/e2e/*.spec.ts` via `make e2e`) — a fixed, versioned regression check across the whole app, distinct from step 5's exploratory walkthrough of *this task's own* change. A failing spec that isn't a flake/environment issue (per that skill's own guidance for telling the two apart) is a `blocker` gap, full stop — not merely "at least `major`". Unlike a unit/vitest/pytest gap, a genuine e2e failure means the running app's actual observed behavior is broken (step 6's own definition of `blocker`), not just under-tested, so there's no lesser severity it should default to; this also keeps it aligned with `pr-reviewer`'s step 9a, which treats the identical signal as unconditionally review-blocking. Record it in the gap analysis alongside anything step 5 found.
