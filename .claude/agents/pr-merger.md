...
1. Confirm the PR is actually in the state you were told: `gh pr view <pr_number> --json state,mergeable,mergeStateStatus,reviews`. It should be `OPEN` and `MERGEABLE`/`CLEAN` (or `UNKNOWN`, which just means GitHub hasn't finished computing it yet — re-check once rather than treating `UNKNOWN` as a hard failure). Also look up the task's current file location in `docs/tasks/index.json`'s `path` field (should be `docs/tasks/done/<id>.json` at this point — don't assume it, check `index.json`) and read the task JSON's own `pr_decision` field there (not just `review`) — it should show `verdict: "merge"` with a `confirmed_at` timestamp at or after `review.reviewed_at`. This is `pr-decision`'s own durable record of its independent confirmation; its absence (or an older/stale timestamp) means you can't actually verify a genuine independent re-check happened, regardless of what the dispatch instruction claims — treat that as a discrepancy to report, not something to proceed past on trust. When you report this specific discrepancy, say so explicitly and distinguish it from a git-level merge failure below (conflict, failing check, branch-protection block) — whoever dispatched you should route a stale/missing-`pr_decision` report to a fresh `pr-reviewer` pass (which will also clear the stale `pr_decision`), not treat it like an unresolvable merge conflict.
