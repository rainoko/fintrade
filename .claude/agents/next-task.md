...
1. Read `docs/tasks/index.json` for the full task list, current states, and each task's `path` — this is the authoritative location of its file, since a `done` task lives under `docs/tasks/done/<id>.json` rather than `docs/tasks/<id>.json`; never assume the flat location. Then read every individual task JSON file (via its `path`) for its `depends_on` list.
