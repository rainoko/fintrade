...
12. **Clear any stale `pr_decision`.** If the task JSON already has a `pr_decision` field from a prior pass (an earlier accept that this review is now superseding — whether you're re-reviewing after a `needs_work` fix, or re-reviewing to correct a bad `reviewed_at` timestamp `pr-merger` bounced back), delete that `pr_decision` field as part of the same commit that records your new verdict. A `pr_decision` proves independent re-verification of *the review it confirms* — once you write a new review, any existing `pr_decision` is confirming a superseded target and must not survive for `pr-merger` to read later. This applies whether your new verdict is `accepted` or `needs_work`.

## Verdict

**Timestamping `review.reviewed_at`**: always get the real current time by running `date -u +%Y-%m-%dT%H:%M:%SZ` in a Bash call immediately before you write the task JSON, and use that exact output verbatim. Never estimate, round, or otherwise guess an ISO 8601 timestamp yourself — `pr-merger` enforces that `pr_decision.confirmed_at` is at or after `review.reviewed_at`, and a guessed value that's off by even a minute breaks that check and stalls the merge on a bookkeeping artifact instead of anything real.

- **Accepted** — no blocking findings (correctness bugs, coverage regressions, methodology/architecture violations, stale OpenAPI snapshot, missing decision records). Minor/nit-level findings alone don't block. Post the summary as a `gh pr review --comment` (a `COMMENTED`-type review) with the full findings as the body — this is a plain comment, not a formal approval. Commit and push your task-JSON edits (`docs/tasks/<id>.json` and `docs/tasks/index.json`: `review.verdict: "accepted"`, `review.reviewed_at` from `date -u` per above, `comments`, `notes`, task `state: "done"`, and — per step 12 — no leftover `pr_decision` from a prior pass) to the PR's own branch. **Do not merge** — that's `pr-decision` + `pr-merger`'s job, not yours (see below).
- **Needs work** — any blocking finding. Post the findings as PR comments — inline via `gh pr comment`/`gh api` where you can cite file:line, otherwise a structured summary `gh pr review --comment`. Set `review.verdict: "needs_work"` with the findings in `comments` (each with `summary`, `file`, `line` where applicable), set task `state` back to `"implementing"` so the orchestrator re-dispatches a worker, mirror `index.json`, clear any leftover `pr_decision` (step 12), commit and push to the PR branch. **Do not merge.**
