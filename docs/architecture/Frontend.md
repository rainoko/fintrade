...
**`resolutions` entry in `package.json`** (`"openapi-typescript/typescript": "$typescript"`): `openapi-typescript@7.13.0`'s published `peerDependencies` still pin `typescript@^5.x`, which conflicts with this project's own `typescript@~6.0.2` and otherwise fails install with `ERESOLVE` (npm) / a peer warning (Yarn) — no released `openapi-typescript` version has widened that range yet (tracked upstream: [openapi-ts/openapi-typescript#2723](https://github.com/openapi-ts/openapi-typescript/issues/2723)). Originally added as an npm `overrides` entry (`frontend-verify-npm-install`, PR #115), where the `$typescript` back-reference genuinely forced npm's resolver to satisfy `openapi-typescript`'s `typescript` peer with this project's own root version instead of its stale declared range. Translated to Yarn's `resolutions` syntax during the npm→Yarn migration (`migrate-yarn2`, PR #116) — but under Yarn, `typescript` is only a `peerDependency` of `openapi-typescript` rather than a real dependency edge, and `resolutions` only rewrites the latter, so the entry likely no longer forces anything there: `yarn install` still emits a `YN0060` peer warning for `openapi-typescript`'s stale `^5.0.0` request even with the entry in place, and peer resolution already converges on a single top-level `typescript@6.0.3` without it. Whether it's safe to remove under Yarn is tracked as an open question in `migrate-yarn2-followups`. Regardless of the entry's effect, the `typescript@6.0.3` actually resolved is verified working end to end: a fresh install is clean and `generate:api-types` regenerates `src/api/types.ts` byte-for-byte identical to the committed file (`frontend-verify-npm-install`'s `decisions`), and a synthetic-schema smoke test covering `oneOf`/`allOf`/`readOnly`/`webhooks` (constructs the current `backend/openapi.json` doesn't exercise) also generates correct, `tsc --strict`-clean output under `typescript@6.0.3` (`frontend-verify-npm-install-followups`'s `decisions`).
