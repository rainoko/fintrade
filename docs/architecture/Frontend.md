...
- **`utils/` is for domain-agnostic *non-component* helpers.** The placement test mirrors `components/common/`'s but for plain functions: domain-agnostic (doesn't reference a feature's domain concepts) and not a component (no props, no rendering, no Storybook story) → `utils/`; domain-agnostic and a component → `components/common/`; references a feature's domain concepts (a "position", a "signal", a Triple Screen field) → `features/<domain>/`, colocated with the components/hooks that use it. `utils/format.ts`'s `humanizeSnakeCase`/`formatNullableNumber` are the motivating example: pure display helpers consumed by both `features/stocks` and `features/portfolio`, with no props/rendering to give them a Storybook story, so neither `features/<domain>/` nor `components/common/` fit.
  - **Named exception: domain-specific content genuinely shared by exactly two feature registries, where neither owns it.** `utils/profitTargetHelpText.ts` (`PROFIT_TARGET_DEFINITION`/`PROFIT_TARGET_ELDER_CONTEXT_SUFFIX`, consumed by both `features/stocks` and `features/portfolio`'s own `metricHelpContent.ts` registries) is genuinely domain-specific text (Elder ch. 58's Tradebill "A"-target formula, Autoenvelope/channel height, ch. 53's 2:1 rule) rather than domain-agnostic — it fails the plain reading of the rule above. It's placed in `utils/` anyway because the alternative is worse: putting it under either feature's `features/<domain>/` would make the *other* feature's registry cross-import a sibling feature's internals, an established anti-pattern nothing else in this codebase does. This is the one deliberate, named carve-out from the domain-agnostic test above, not a precedent for putting ordinary single-feature domain content in `utils/` (`utils/insiderClusters.ts` is a single-feature helper and doesn't need this exception). If a second case with this same shape (content two-or-more features' registries need, that no one feature owns) shows up, prefer this same pattern over inventing a new location each time.
...
Both `PriceChart.tsx` and `OscillatorChart.tsx` also draw `GET /api/stocks/{ticker}/analysis`'s `divergence` field (`frontend-divergence-markers`) — the single most-recently-qualifying MACD-Histogram/Stochastic/RSI divergence, not a reconstructed history of every past one (see the task's `decisions` entry). `PriceChart.tsx` connects the two extremes' own *prices* with a dashed `LineSeries` plus a `circle` marker at each end (`theme.palette.divergence.main`, a color/shape deliberately distinct from the BUY/SELL arrow markers); `OscillatorChart.tsx` connects the same two extremes' own *indicator readings* instead, on whichever pane that indicator lives on (Stochastic/RSI's shared pane 0, or MACD Histogram's pane 2). Both wire `chart.subscribeClick` so clicking either marker opens `common/InfoBalloon`'s new `AnchoredInfoBalloon` export — a coordinate-anchored variant of the same title+content balloon, for a canvas-drawn marker with no DOM trigger element of its own to attach the usual `ButtonBase`/`onClick` pattern to. Both also add a `divergenceHelp` (`metricHelpContent.ts`) legend row with the usual `common/MetricHelp` affordance, reading `interpretValue` content that names the actual two dates/values compared for the current ticker (not a generic definition) — reachable via the legend icon in addition to a direct marker click, so the explanation doesn't depend on precisely hitting a small canvas target.
...
`rsi` (`frontend-rsi-oscillator-chart`) is plotted as a second dashed `LineSeries` sharing Stochastic %K's own pane 0/y-axis, not a fourth separate pane — both are bounded 0-100 oscillators, and the whole point of adding RSI here is Elder's own side-by-side comparison (Analyse.md row 10, ch. 27): RSI is closing-price-only and tends to signal earlier/cleaner than Stochastic (which also reads each bar's high/low), a comparison that only reads clearly when both lines share one axis. A `rsiHelp` `common/MetricHelp` affordance in a legend row above the chart states this comparison explicitly and interprets the current RSI value against the current Stochastic %K reading, per this app's established explanatory pattern (`metricHelpContent.ts`).
