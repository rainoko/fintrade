...
**`resolutions` entry in `package.json`** (`"openapi-typescript/typescript": "$typescript"`): `openapi-typescript@7.13.0`'s published `peerDependencies` still pin `typescript@^5.x`, which conflicts with this project's own `typescript@~6.0.2` and otherwise fails install with `ERESOLVE` (npm) / a peer warning (Yarn) — no released `openapi-typescript` version has widened that range yet (tracked upstream: [openapi-ts/openapi-typescript#2723](https://github.com/openapi-ts/openapi-typescript/issues/2723)). The `$typescript` back-reference forces `openapi-typescript` to resolve `typescript` against this project's own root version instead of its stale declared peer range. Originally added as an npm `overrides` entry (`frontend-verify-npm-install`, PR #115); translated to Yarn's `resolutions` syntax during the npm→Yarn migration (`migrate-yarn2`, PR #116) — see `migrate-yarn2-followups` for an open question on whether this specific entry still does anything under Yarn's resolution semantics (peer-only edges, unlike npm's, may not need it). Verified working end to end: a fresh install is clean, `generate:api-types` regenerates `src/api/types.ts` byte-for-byte identical to the committed file, and a synthetic-schema smoke test covering `oneOf`/`allOf`/`readOnly`/`webhooks` (constructs the current `backend/openapi.json` doesn't exercise) also generates correct, `tsc --strict`-clean output under `typescript@6.0.3` — see `frontend-verify-npm-install-followups`'s `decisions` for the full verification record.
