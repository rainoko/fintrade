...
Query params: `range` (same grammar as `/history`'s `range` — `<N>d` | `<N>w` | `<N>m` | `<N>y` | `max`, default `1y`). Daily bars only while `swing` mode is active — no `interval` param, since every indicator/Screen this endpoint computes is itself daily-cadence in that mode; while `day_trader` mode is active, one bar per the active triple's own *intermediate*-leg width instead (see `trading_mode` below).

The full computed response is cached, keyed by `(ticker, range)`, with a same-*calendar-day* TTL (`app.api.indicator_history_cache.IndicatorHistoryResponseCache`, backed by the `indicator_history_cache` table alongside `ohlcv_cache`/`extended_data_cache` — Backend.md §7) — a repeat same-day request for the same ticker/range skips both the OHLCV fetch and the whole per-bar Triple Screen recompute below entirely. Only a successful response is ever cached, never an error. On a cache miss, daily and weekly OHLCV are fetched concurrently rather than sequentially — see the `backend-indicator-history-performance` task's `decisions` for both the TTL-shape choice (calendar-day, not a rolling window like `ohlcv_cache`'s own 24h TTL) and why that existing OHLCV TTL itself was deliberately left unchanged. This same-day response cache can therefore serve a signal computed from an earlier-in-the-day OHLCV snapshot for the rest of that calendar day even after `ohlcv_cache` itself has since been refreshed (e.g. by another endpoint's request, or today's daily bar settling after market close) — accepted for the same reason as `ohlcv_cache`'s own 24h TTL tradeoff; see the task's `-followups` entry. **This whole cache is bypassed entirely (never read or written) while `day_trader` mode is active** (`backend-day-trader-timeframe-mode-api-followups`) — a whole-calendar-day TTL designed for swing mode's once-a-day cadence would serve an actively-wrong (not just stale-by-degree) response for intraday data that can change every few minutes; every day-trader-mode request is computed fresh, matching `/analysis`/`/watchlist`'s own always-live day-trader-mode convention.
...
`trading_mode` (`backend-day-trader-timeframe-mode-api-followups`) is the same `TradingModeOut` shape as `/analysis`'s own `trading_mode` field. While `swing` (below, unchanged), `points` is produced exactly as documented in this section. While `day_trader` with a configured triple: `app.api.day_trader_signal.fetch_day_trader_history_legs` fetches the triple's three legs via IBKR and `app.signals.engine.analyse_history_day_trader` replays the walk-forward Triple Screen evaluation over the *intermediate* leg (playing this section's own `daily_ohlcv`/driving-leg role), with the *long-term* leg playing Screen 1/Tide's role the same way `weekly_ohlcv` does in swing mode — this endpoint raises `503` (see Errors below) rather than ever returning a partial/degraded body when that data isn't available right now, same convention as `/analysis`'s own day-trader branch. `IndicatorHistoryPoint.date` is still a plain calendar date (not a full timestamp) in this mode — several consecutive `points` entries can share the same `date` whenever the intermediate leg's own bar width is finer than one calendar day (e.g. several 10-minute bars); this is a known, accepted limitation (no frontend can reach day-trader mode yet to need finer resolution) rather than a widened/renamed field — see this task's `decisions` entry for the same non-breaking-change rationale `backend-day-trader-timeframe-mode-api`'s own field-naming decision already established. `range`'s existing grammar isn't extended with a minute/hour unit either, for the same reason — a day-trader-mode chart can only be trimmed at day-or-coarser resolution via `range` today.

`points` is oldest-first, one entry per daily bar in the requested range while `swing` mode is active (see `trading_mode` above for `day_trader` mode's own intermediate-leg-bar cadence), produced by re-running the signal engine (`app.signals.engine.analyse`) once per bar using only that bar's own history — including Screen 1 (Tide), which is recomputed from only the weekly bars as-of that day's own calendar week (`app.signals.engine._long_term_through_bar_date`), not held fixed at today's value — so `signal`/`confidence`/Tide all genuinely vary day to day, not just the underlying daily indicators, with no look-ahead. The last entry matches `GET /api/stocks/{ticker}/analysis` for the same ticker at the same date whenever both are computed fresh from the same untruncated inputs: for the most recent daily bar, "the weekly bars as-of that bar's calendar week" naturally reduces to the full weekly series `/analysis` itself uses — see the `api-stocks-indicator-history` task's `decisions` for the full rationale (including why an earlier, simpler `<= bar_date` truncation attempt would have broken that "last entry matches `/analysis`" guarantee, given how the underlying weekly-resample date labeling works). This can still drift for the rest of a calendar day once this endpoint's own same-day response cache (above) is serving a hit computed from an earlier OHLCV snapshot than `/analysis`'s own always-fresh call — see this task's `-followups` entry.
...
`trading_mode` (`backend-day-trader-timeframe-mode-api-followups`) is the same `TradingModeOut` shape as `/analysis`'s own `trading_mode` field. `current_price`/`unrealized_pnl_pct`/`equity` are always derived from the ordinary swing daily chart regardless of this field — see this field's own description on `AnalysisResponse` for the identical `as_of`/`extended_data`/`profit_target`-stay-swing-derived precedent. Only `signal`/`confidence`/`confidence_band` change with the mode.

Each position is also annotated with its current `signal`/`confidence`/`confidence_band`. While `swing` mode is active: via the exact same Triple Screen signal engine `GET /api/stocks/{ticker}/analysis` and `GET /api/watchlist` use (`app.signals.engine.analyse`, Analyse.md §5) — not a separately-implemented buy check, reusing the same per-position market-data fetch `current_price` is derived from. `signal`/`confidence`/`confidence_band` are `null` together on a position whose signal couldn't be computed right now — either its `current_price` fetch already failed (same condition as `current_price`/`unrealized_pnl_pct` above), that fetch succeeded but the separate weekly-history fetch the signal engine additionally needs (for Screen 1/Tide) failed, or the latest daily bar has a valid close (so `current_price` is still available) but NaN open/high/low and so doesn't survive the signal engine's stricter filtering (`app.signals.engine.drop_malformed_daily_bars`) — mirroring `WatchlistItemOut`'s null-on-failure pattern rather than failing the whole request or dropping the position (see the `api-portfolio-position-signal` task's `decisions`). While `day_trader` mode is active with a configured triple (`backend-day-trader-timeframe-mode-api-followups`): `app.signals.engine.analyse_day_trader` over the active triple's three IBKR-fetched legs instead — every held position's ticker is resolved *concurrently* up front, not one IBKR round trip at a time (`app.api.day_trader_signal.compute_day_trader_signals_concurrently`). `signal`/`confidence`/`confidence_band` go `null` for the same day-trader-mode reasons `WatchlistItemOut.signal` documents (IBKR disabled/unreachable/unauthenticated, this ticker's IBKR contract id not resolving, or a non-fully-intraday triple) — deliberately **not** additionally gated on this same position's own swing `current_price` fetch succeeding, unlike the swing case above, since the two are entirely independent data sources in this mode (a contrast with `/analysis`'s own day-trader branch, which still fetches daily/weekly before checking the active mode — see this task's `decisions` entry).
...
`trading_mode` (`backend-day-trader-timeframe-mode-api-followups`) is the same `TradingModeOut` shape as `/analysis`'s own `trading_mode` field. While `swing` mode is active, every per-position field above is computed exactly as documented in this section, from that ticker's ordinary daily/weekly OHLCV. While `day_trader` mode is active with a configured triple: `protective_stop`/`trailing_stop`/`exit_flags`/`profit_target` are instead computed from the active triple's intermediate/long-term legs (fetched via IBKR, concurrently across every held position up front — `app.api.day_trader_signal.fetch_day_trader_legs_concurrently`) — `app.portfolio.risk.protective_stop`/`app.portfolio.profit_target.suggest_profit_target`/`app.portfolio.exits.evaluate_exit_flags` need no branching of their own for this, since all three already generalize to whichever OHLCV plays the intermediate/long-term role (`backend-day-trader-timeframe-mode-portfolio-risk`). A position whose day-trader-mode legs aren't available right now (same reasons as `WatchlistItemOut.signal`'s day-trader-mode paragraph) is silently excluded from `positions`, the same as any other can't-be-computed-right-now reason above. `total_open_risk_pct`/`realized_losses_this_month_pct`/`six_percent_rule_breached` are unaffected either way — portfolio-wide account-equity/closed-trade computations with no per-position timeframe dependency.
...
The global, app-wide active trading mode (docs/tasks/backend-day-trader-timeframe-mode.json, Elder ch. 39's "Choosing Timeframes — the Factor of Five") — a single setting, not a per-request parameter (see that task's `decisions` entry). Switching to `day_trader` mode here now changes every signal-facing endpoint's computed output for a **fully-intraday** triple: `GET /api/stocks/{ticker}/analysis`/`GET /api/watchlist`/`GET /api/watchlist/breadth` (`backend-day-trader-timeframe-mode-api`) and `GET /api/stocks/{ticker}/indicators`/`GET /api/portfolio`/`GET /api/portfolio/risk` (`backend-day-trader-timeframe-mode-api-followups`) — see each endpoint's own `trading_mode` documentation above. A mixed (non-fully-intraday) triple isn't supported by any of them yet — see `docs/architecture/Backend.md` §10 for what's landed vs. still tracked as a dependent follow-up task.
...
- `day_trader` trading mode active but its intraday data can't be fetched right now (IBKR disabled/unreachable/unauthenticated, a ticker's IBKR contract id not resolving, or a non-fully-intraday triple) → `503` on `GET /api/stocks/{ticker}/analysis`/`GET /api/stocks/{ticker}/indicators` (single-ticker endpoints, so this is a failed request); `null` `signal`/`confidence`/`confidence_band` (or `unavailable_count`) on `GET /api/watchlist`/`GET /api/watchlist/breadth`/`GET /api/portfolio`, and silent exclusion from `positions` on `GET /api/portfolio/risk` (multi-ticker/multi-position endpoints, so this never fails the whole request) — see each endpoint's own `trading_mode` documentation above.
