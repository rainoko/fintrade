...
class ProfitTargetOut(BaseModel):
    ...
    distance_to_target: float = Field(description="`price` minus current close -- the per-share potential reward from here at current close. Always > 0 by construction (both target techniques only ever produce a price above current close).")
