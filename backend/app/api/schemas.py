...
class ExtendedDataOut(BaseModel):
    ...
    insider_transactions: list[InsiderTransactionOut] = Field(description="Recent officer/director buy/sell filings (`Ticker.insider_transactions`, Elder ch. 37), in the order yfinance itself returns them (most-recent-first). Empty when none are reported, or when `unavailable_reason` is set below -- an empty list either way, since 'no filings' and 'not checked' aren't distinguished at the per-field level (only `unavailable_reason` itself distinguishes them). Raw exposure only -- clustering detection (3+ buys or sells within a month, Elder's own secondary signal) isn't computed here; see the backend-market-data-extra-fields task's `decisions` entry.")
...
class ProfitTargetOut(BaseModel):
    ...
    source: Literal["channel", "support_resistance"] = Field(description="Which technique produced `price`: 'channel' -- ch. 58's own Tradebill formula, current close + 30% of today's Autoenvelope/channel height (same channel `AnalysisResponse.indicators.channel_upper`/`channel_lower` expose); 'support_resistance' -- the nearest `support_resistance_zones` level above current close. Whichever of the two is TIGHTER (closer to current close) is used -- see the backend-profit-target task's `decisions` entry for why the more conservative target is preferred over a fixed preference order.")
