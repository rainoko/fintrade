...
class TestAnalyseHistory:
    ...
    def test_weekly_ohlcv_is_never_truncated_tide_constant_across_series(self) -> None:
        """Screen 1 (Tide) is weekly-derived and, per this function's own documented
        decision, is held at its current (full weekly_ohlcv) value for every point in the
        series rather than recomputed from a per-day-truncated weekly frame -- see this
